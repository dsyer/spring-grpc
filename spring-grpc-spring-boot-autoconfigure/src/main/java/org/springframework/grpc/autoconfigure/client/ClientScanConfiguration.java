...
import org.springframework.grpc.client.GrpcClientConfiguration;
import org.springframework.grpc.client.GrpcClientRegistryCustomizer;

import io.grpc.stub.AbstractBlockingStub;

@Configuration(proxyBeanMethods = false)
@ConditionalOnMissingBean(GrpcClientConfiguration.class)
@EnableGrpcClients
	...
	public GrpcClientRegistryCustomizer defaultGrpcClientRegistryCustomizer(BeanFactory beanFactory,
		...
		return registry -> {
			if (hasDefaultChannel) {
				registry.channel("default").scan(AbstractBlockingStub.class, packages.toArray(new String[0]));
			}
		};
	}
